      ...
      <ul className='"person-items'>
        {persons.map(person => (
          <li key={person.id} className='person-item'>  {/*The key prop helps React identify which items have changed, are added, or are removed.*/}
            ...
            <butoon type="butotn" className="btn btn-cancel" onClick={() => setEditingPerson(null)}>cancel</butoon> {/*The Cancel button resets editingPerson to null, exiting edit mode.*/}
          </form>
        ):(
        <>
          ...
          <div className='person-actions'>
            ...
            <button className='btn btn-delete' oncliclick={() => deletePerson(person.id)}>DELETE</button>
          </div>
        </>
                 )}
      </li>
